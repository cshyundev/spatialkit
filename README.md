...
- **프로토타이핑 및 연구를 위한 테스트 코드**: 컴퓨터 비전 알고리즘 개발 및 테스트 단계에서 자주 필요한 테스트 코드를 포함한다.
...
### 2. 호모그래피 변환
이미지의 관심있는 부분을 호모그래피(Homography) 변환을 통해 추출하고 결과를 보여준다.\
[호모그래피 변환 테스트 사용법](docs/tests/homography_transformation.md)

### 3. 이미지 왜곡 보정 변환
카메라로 촬영된 이미지의 왜곡을 보정한다.\
[이미지 왜곡 보정 테스트 사용법](docs/tests/image_distortion_correction.md)

### 4. 카메라 캘리브레이션 정확도 테스트 (개발 예정)
카메라 캘리브레이션 결과를 재투영 오차(reprojection error)와 왜곡 보정을 통해 정량적, 정석적으로 평가한다.


### 5. COLMAP을 이용한 3차원 재구성 (개발 예정)
pycolmap을 이용해 이미지 데이터셋으로 부터 3차원 재구성(3D Reconstruction)을 수행하고, 결과를 저장한다.

### 6. Fiducial Marker 인식률 및 Pose 정확도 테스트 (개발 예정)

...
def is_quat(x: ArrayLike) -> bool:
    ...
    """
    shape = x.shape
    if shape != (4,): return False # invalid shape
    return isclose(norm(x),1.)
...
def quat_to_SO3(quat: ArrayLike, is_xyzw: bool) -> ArrayLike:
    ...
    """
    is_quat(quat), (f"Input is not satisfied to quaternion properties.")
...
def SO3_to_so3(SO3: ArrayLike) -> ArrayLike:
    """
        ...
        Return:
            so3 (ArrayLike, [3,]): so3 vector
    """
    assert is_SO3(SO3), (f"Input is not satisfied to SO3 properties.")
...
class Rotation:
    ...
    def __mul__(self, other: Any) -> Union['Rotation',ArrayLike]:
        ...
        elif is_array(other): return self.apply_pts3d(other)
        else:

...
class Camera:
    ...
    def __init__(self, cam_dict: Dict[str, Any]):
        ...
        self._max_fov_deg = cam_dict.get("fov_deg", -1.)
    ...
    def set_mask(self, mask:ArrayLike):
        if mask.shape != self.hw:
            LOG_ERROR(f"Mask's shape must be same as image size{self.hw}, but Mask's shape = {mask.shape}.")
    ...
    def export_cam_dict(self) -> Dict[str,Any]:
        ...
        if self._max_fov_deg > 0: cam_dict["fov_deg"] = self._max_fov_deg
        return cam_dict
    
    def _warp(self, image:ArrayLike, uv:ArrayLike, valid_mask:Optional[ArrayLike]=None) -> ArrayLike:
        """
...
class RadialCamera(Camera):
    ...
    def _to_image_plane(self, x:ArrayLike, y:ArrayLike) -> Tuple[ArrayLike,ArrayLike]:
        u = self.fx * x + self.skew * y + self.cx
        v = self.fy * y + self.cy
        return u,v
    
    def _to_normalized_plane(self, uv:ArrayLike) -> Tuple[ArrayLike,ArrayLike]:
        x = (uv[0:1,:] - self.cx - self.skew / self.fy*(uv[1:2,:] -self.cy)) / self.fx # (1,N)
    ...
    def _compute_residual_jacobian(self,xu:ArrayLike,yu:ArrayLike, xd:ArrayLike, yd:ArrayLike) \
    ...
    def _compute_fov_mask(self) -> ArrayLike:
        ...
        fovx, fovy = self.fov
        max_theta = deg2rad(np.max(fovx,fovy)) / 2.
    ...
    def undistort_pixel(self, uv:ArrayLike,out_subpixel:Optional[bool]=False) -> ArrayLike:
        if self.has_distortion() is False: return uv
        x,y = self._to_normalized_plane(uv)        
...
class PerspectiveCamera(RadialCamera):
    ...
    def from_K(K: List[List[float]], image_size:List[int], dist_coeffs: Optional[List[float]]=None) -> 'PerspectiveCamera':
        ...
        if dist_coeffs is not None:
            if len(dist_coeffs) != 5:
                LOG_CRITICAL(f"The distortion factor must be 5, but got {len(dist_coeffs)}")
            cam_dict["radial"] = (dist_coeffs[0],dist_coeffs[1],dist_coeffs[4]) # k1, k2, k3
    ...
    def _distort(self, x: ArrayLike, y: ArrayLike) -> Tuple[ArrayLike,ArrayLike] :
        ...
        R = 1. + r2 * (k1 + r2 * (k2 + k3 *r2)) # Radial distortion R(x,y)
    ...
    def _compute_residual_jacobian(self,xu:ArrayLike,yu:ArrayLike, xd:ArrayLike, yd:ArrayLike) \
        ...
        k1,k2,k3 = self.radial_params
        r2 = xu**2 + yu**2
        R = 1. + r2 * (k1 + r2 * (k2 + k3 *r2)) # radial distortion term R(.,.)
        dRdr = 2 * (k1 + r2 * (2. * k2 + 3. * k3 * r2)) # ∂R/∂r * 1/r
        dRdx = dRdr * xu # ∂R/∂x = ∂R/∂r * ∂r/∂x = ∂R/∂r * x/r 
        dRdy = dRdr * yu # ∂R/∂y = ∂R/∂r * ∂r/∂y = ∂R/∂r * y/r

        p1, p2 = self.tangential_params
        # Compute derivative of distorted point(xd) over x and y 
        res_x_x = R + dRdx * xu + 2. * p1 * yu + 6. * p2 * xu # R + ∂r/∂x + ∂(Tx)/∂x
        res_x_y = dRdy * xu + 2. * p1 * xu + 2. * p2 * yu # ∂R/∂y * x + ∂(Tx)/∂y
        # Compute derivative of distorted point(yd) over x and y        
        res_y_x = dRdy * yu + 2.0 * p2 * yu + 2. * p1 * xu # ∂R/∂x * y + ∂(Ty)/∂x
        res_y_y = R + dRdy * yu + 2. * p2 * xu + 6. * p1 * yu # R + ∂R/∂y * y + ∂(Ty)/∂y
...
class OpenCVFisheyeCamera(RadialCamera):
    """
        ...
        Attributes:
            ...
            radial (List[float]): Radial distortion parameters [k1, k2, k3, k4], specifying the lens's radial distortion. Typically used in distortion correction algorithms.
    ...
    @staticmethod
    def from_K_D(K: List[float], image_size:List[int], D: List[float]) -> 'OpenCVFisheyeCamera':
        """
    ...
    def _distort(self, x: ArrayLike, y: ArrayLike) \
        ...
        R = 1. + r2 * (k1 + r2 * (k2 + r2* (k3 + r2 * k4))) # Radial distortion term
        xd = R * x
        yd = R * y
    ...
    def _compute_residual_jacobian(self,xu:ArrayLike,yu:ArrayLike, xd:ArrayLike, yd:ArrayLike) \
        ...
        r2 = xu**2 + yu**2
        R = 1. + r2 * (k1 + r2 * (k2 + r2* (k3 + r2 * k4))) # Radial distortion term
        dRdr =  2 * (k1 + r2 * (2. * k2 + r2 * (3 * k3 + 4. * k4 * r2))) # ∂R/∂r * 1/r
        dRdx = 2. * dRdr * xu # ∂R/∂x = ∂R/∂r * ∂r/∂x = ∂R/∂r * x/r
        dRdy = 2. * dRdr * yu # ∂R/∂y = ∂R/∂r * ∂r/∂y = ∂R/∂r * y/r
...
class ThinPrismFisheyeCamera(RadialCamera):
    ...
    def _compute_residual_jacobian(self,xu:ArrayLike,yu:ArrayLike, xd:ArrayLike, yd:ArrayLike) \
        ...
        sx1, sy1 = self.prism_params
        # Compute derivative of distorted point(xd) over x and y 
        res_x_x = R + dRdx * xu + 2. * p1 * yu + 6. * p2 * xu + 2. * sx1 * xu  # R + ∂r/∂x + ∂(Tx)/∂x + ∂(Sx)/∂x 
        res_x_y = dRdy * xu + 2. * p1 * xu + 2. * p2 * yu + 2. * sx1 * yu # ∂R/∂y * x + ∂(Tx)/∂y + ∂(Sx)/∂y 
        # Compute derivative of distorted point(yd) over x and y        
        res_y_x = dRdy * yu + 2.0 * p2 * yu + 2. * p1 * xu + 2. * sy1 * xu # ∂R/∂x * y + ∂(Ty)/∂x + ∂(Sy)/∂x 
        res_y_y = R + dRdy * yu + 2. * p2 * xu + 6. * p1 * yu + 2. * sy1 * yu # R + ∂R/∂y * y + ∂(Ty)/∂y + + ∂(Sy)/∂y 
...
class DoubleSphereCamera(Camera):
    ...
    def convert_to_rays(self, uv: Optional[ArrayLike]=None) -> Tuple[ArrayLike,ArrayLike]:
        ...
        mask = logical_and(mask, fov_mask.reshape(-1,), valid_mask.reshape(-1,))

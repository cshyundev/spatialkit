...
def parse_scannetv1_dataset(dataset_path: str):
    ...
    if len(image_paths) != len(depth_paths) or len(image_paths) != len(pose_paths) or len(depth_paths) != len(pose_paths):
        if len(image_paths) != len(depth_paths):
            LOG_ERROR(f"The number of images and depths must be same.")
        if len(image_paths) != len(pose_paths):
            LOG_ERROR(f"The number of images and poses must be same.")
        if len(depth_paths) != len(pose_paths):
            LOG_ERROR(f"The number of depths and poses must be same.")   
        return None
    ...
    def parse_txt(file_path):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                if ' ' in value:
                    if value == 'StructureSensor (calibrated)': continue # unused value

...
It includes methods for loading, preprocessing, and manipulating the dataset for 3D reconstruction and scene understanding tasks.
...
from ...externals.o3d.common import O3dPCD, O3dTriMesh
from ...externals.o3d import o3dutils  
...
class BaseMVSManger:
    ...
    def get_cam(self, id:int) -> Camera:
        ...
        """
        if self.is_single_cam: return self.cam
        else: return self.cams[self.indices[id]]
    ...
    def compute_point_cloud(self, ids:Optional[List[Any]]=None) -> O3dPCD:
        ...
        pcds, colors = [], []
        for id in ids:
            depth,image,tf,cam = self.get_depth(id), self.get_image(id), self.get_pose(id), self.get_cam(id)
...
class ScannetV1Manager(BaseMVSManger):
    ...
    def __init__(self,dataset_path:str, use_buffer:bool=False) -> None:
        ...
        if dataset_dict is None:
            LOG_ERROR(f"Failed to load scannet dataset files.")
            return
        
        self.cam = PerspectiveCamera.from_K(dataset_dict["image_calib"],dataset_dict["image_size"])
        self.is_single_cam = True
        self.has_depth = True

        self.depth_cam = PerspectiveCamera.from_K(dataset_dict["depth_calib"],dataset_dict["depth_size"])
        self.depth_shift = dataset_dict["depth_shift"]
        
        world2cam = Transform.from_mat(dataset_dict["m_calibrationColorExtrinsic"]).inverse()
        ...
        if use_buffer:
            ...
            for id in self.ids:
                self.image_buffer[idx] = self.get_image(id)
                self.depth_buffer[idx] = self.get_depth(id)
        self.use_buffer = use_buffer
        
    def get_depth(self, id: int):
        index = self.indices[id]
        if self.use_buffer: return self.depth_buffer[index]

        depth_map = io.read_pgm(self.depth_paths[index],mode='F') / self.depth_shift
        # align depth map into color image
        pcd = geou.convert_depth_to_point_cloud(depth_map,self.depth_cam,map_type='MPI',pose=self.depth2cam)
        aligned_depth = geou.convert_point_cloud_to_depth(pcd, self.get_cam(id), map_type='MPI')

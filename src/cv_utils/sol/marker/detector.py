...
Description:
    ...
    inherited for detecting various fiducial markers (e.g., ArUco, AprilTag).
...
class MarkerDetector:
    ...
    Note:
        - By default, the marker is assumed to lie on the Z=0 plane, with its center
          at the origin (0,0,0).
        The order of the 3D corners is (1) -> (2) -> (3) -> (4)
    ...
    def __init__(self,
        ...
        """
        Initialization of MarkerDetector with a camera, marker size, and ArUco dictionary type.

        Args:
            cam (Camera): A camera object containing intrinsic/extrinsic parameters.
            marker_size (float): The size of the marker's side length in meter.
            marker_type (FiducialMarkerType): The specific ArUco dictionary type to use.
        """
        # Arguments Check
        assert(isinstance(cam,Camera)),(f"cam must be Camera type, but got {type(cam)}.")
        assert(isinstance(marker_size,float)),(f"marker_size must be float type, but got {type(marker_size)}.")
        assert(isinstance(marker_type,FiducialMarkerType)),(f"marker_type must be FiducialMarkerType, but got {marker_type}.")
        assert(marker_size > 0.), ("marker size must be positive.")
    ...
    def detect_marker(self, image:ArrayLike,
                      estimate_pose:bool = True) -> List[Marker]:
        """
        ...
        Args:
            ...
            estimate_pose (bool): If True, try to estimate pose using PnP Algorithm
    ...
    def draw_axes(self,
        ...
        """
        Draws the local X, Y, Z axes of a marker onto the image. 
        ...
        Returns:
            np.ndarray:
                ...
                axes are different intensities; for color images, 
                they are drawn in BGR colors.

        Notes:
            - We define 4 points in marker-local space:
                * [0, 0, 0]             : Origin
                * [axis_length, 0, 0]   : X-axis end
                * [0, axis_length, 0]   : Y-axis end
                * [0, 0, axis_length]   : Z-axis end
            - We transform those points into the camera frame, then 
              project them to 2D pixel coords via self._cam.convert_to_pixels.
            - If single-channel, we draw lines with distinct gray intensities:
                * X-axis: 255   (white)
                * Y-axis: 170   (lighter gray)
                * Z-axis: 85    (darker gray)
            - If multi-channel, we assume BGR order:
                * X-axis: (255, 0, 0)   (red)
                * Y-axis: (0, 255, 0)   (green)
                * Z-axis: (0, 0, 255)   (blue)
        """
        ...
        # Convert to (x,y) integer coords for line drawing
        ...
        if is_grayscale:
            ...
            # X -> 255, Y -> 170, Z -> 85 (just an example)
            ...
            cv.line(out_img, origin, x_axis, (255, 0, 0), thickness)   # X -> RED
            cv.line(out_img, origin, y_axis, (0, 255, 0), thickness)   # Y -> GREEN
            cv.line(out_img, origin, z_axis, (0, 0, 255), thickness)   # Z -> BLUE
    ...
    def draw_markers(self, 
        ...
        Args:
            ...
            draw_axes (bool): if True, FrameAxes is drawed.
        ...
        Returns:
            np.ndarray:
                ...
                - Single-channel if input was single-channel.
                - Multi-channel if input was color.
        """
        ...
        for marker in markers:
            ...
            # Put ID on left top
            if is_grayscale:
                cv.putText(out_img, f"{marker.id}", text_pos,
                           cv.FONT_HERSHEY_SIMPLEX, 0.7, 170, thickness)
                out_img = draw_polygon(out_img,corners,170,2)
            else:
                cv.putText(out_img, f"{marker.id}", text_pos,
                           cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), thickness)
            ...
            if draw_axes:
                ...
                out_img = self.draw_axes(out_img, marker, axis_length=axis_length, thickness=thickness)
        return out_img
...
class OpenCVMarkerDetector(MarkerDetector):
    ...
    def __init__(self,
        ...
        Args:
            cam (Camera): A camera class.
            marker_size (float): The size of the marker's side length in meter.
            marker_type (FiducialMarkerType): The specific OpenCV dictionary type to use.

        Raises:
            ValueError: If the marker_type is not recognized as an OpenCV type.
        """
        ...
        if marker_type not in self._VALID_OPENCV_TYPES_MAP.keys():
            raise ValueError(f"{marker_type} is not a valid dictionary type opencv supported.")

        _opencv_dict = cv.aruco.getPredefinedDictionary(self._VALID_OPENCV_TYPES_MAP[marker_type]) 
        _opencv_detect_params = cv.aruco.DetectorParameters()
        self._opencv_detector = cv.aruco.ArucoDetector(_opencv_dict,_opencv_detect_params)

    def detect_marker(self, image:ArrayLike,
                      estimate_pose:bool = True) -> List[Marker]:
        
        assert(is_array(image)),(f"image must be ArrayLike(Numpy or Torch), but got {type(image)}.")
        image = convert_numpy(image)
        # Detect markers
        corners, ids, _ = self._opencv_detector.detectMarkers(image)
        ...
        for marker_corners, marker_id in zip(corners, ids):
            ...
            if estimate_pose:
                # SolvePnP
                marker2cam:Transform = solve_pnp(swapaxes(corner_2d,0,1),
                                                 swapaxes(self.corner_3d,0,1),self._cam)
                if marker2cam is None: continue
                # Build Marker object
...
class AprilTagMarkerDetector(MarkerDetector):
    ...
    def __init__(self, cam:Camera, 
        ...
        super(AprilTagMarkerDetector,self).__init__(cam, marker_size, marker_type)

        if marker_type in self._DT_SUPPORT_APRILTAG_TYPES_STR.keys():
        ...
        elif marker_type in self._OPENCV_SUPPORT_APRILTAG_TYPES.keys():
            _opencv_dict = cv.aruco.getPredefinedDictionary(self._OPENCV_SUPPORT_APRILTAG_TYPES[marker_type]) 
            _opencv_detect_params = cv.aruco.DetectorParameters()
            self._apriltag_detector = cv.aruco.ArucoDetector(_opencv_dict,_opencv_detect_params)
            self._use_opencv_deatector = True
            LOG_INFO(f"This MarkerType is processed to OpenCV.")
        else:
    ...
    def detect_marker(self, image:ArrayLike,
        ...
        if self._use_opencv_deatector:
            ...
            if image.ndim == 3: image = cv.cvtColor(image, cv.COLOR_RGB2GRAY)
            detections:List[Detection] = self._apriltag_detector.detect(image,estimate_tag_pose=False)

            if len(detections) == 0: return []
            corners = []
            ids = []
            for detection in detections:
                corners.append(detection.corners[::-1]) # Reverse corner's order. See comment of MarkerDetector Abstract Class
                ids.append(detection.tag_id)

        detected_markers = []

        for marker_corners, marker_id in zip(corners,ids):
            corner_2d = marker_corners[0] if self._use_opencv_deatector else marker_corners
            
            if estimate_pose:
                # SolvePnP
                marker2cam:Transform = solve_pnp(swapaxes(corner_2d,0,1),
                                                 swapaxes(self.corner_3d,0,1),
                                                 self._cam)
                if marker2cam is None: continue
                # Build Marker object
                marker = Marker(marker_id,marker2cam,corner_2d)
...
class STagMarkerDetector(MarkerDetector):
    ...
    def __init__(self, cam:Camera, 
       ...
       # Check if the provided marker_type is a valid dictionary
        if marker_type not in self._VALID_APRILTAG_TYPES_HD.keys():
            raise ValueError(f"{marker_type} is not a valid dictionary type stag supported.")
